...
提取原则：
1. **长期价值**：仅提取反映用户偏好、技术栈、背景背景、习惯或重要事实的信息。忽略情绪波动、天气等瞬时或琐碎信息。
2. **冲突识别**：若发现新事实与“当前记忆列表”冲突（如用户更改了偏好），请提取最新事实，我们会后续处理。
3. **原子化**：每条信息应是一个独立、完整的语义单元，不超过 20 字。
4. **去冗余**：不要提取已存在于“当前记忆列表”中的重复信息。
