...
## 简介
本文件为“待办事项”服务的完整 API 接口规范，覆盖 RESTful 端点、数据传输对象（DTO）、认证与授权、分页/排序/过滤、错误响应格式、请求与响应示例，以及版本控制与向后兼容策略。接口基于 NestJS 构建，采用 JWT 令牌进行认证，使用 Prisma 访问数据库，并通过 Socket 事件实现多端同步通知。
...
- 后端应用位于 apps/backend，核心模块包括：
  ...
  - 数据访问：prisma.service
  - 公共类型与共享 Schema：packages/shared/schemas/todo.schema.ts
...
- 同步链路：TodoSyncService 在单事务中处理客户端推送变更，再拉取服务器端自上次同步以来的变更，合并后返回；同时广播同步通知。
- 错误链路：AllExceptionsFilter 捕获异常，输出统一结构化错误响应，支持国际化。
...
CheckVersion --> |是| Upsert["upsert 本地变更到服务端"]
...
Merge --> Return["返回 synced/deletedIds/acceptedIds/conflicts/serverTime"]
Return --> End(["结束"])
```
...
## 结论
本规范明确了待办事项 API 的端点、数据模型、认证授权、同步机制与错误处理策略。通过共享 Schema 与严格的 DTO 校验，确保前后端一致性；通过 JWT 与全局守卫保障安全；通过事务与事件机制提升可靠性与实时性。建议在生产环境中启用 HTTPS、限流与监控，并持续演进版本与兼容策略。
