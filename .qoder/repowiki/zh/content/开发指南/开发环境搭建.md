...
- 新增 Docker 环境下文件监控改进章节，介绍 VITE_USE_POLLING 和 CHOKIDAR_USEPOLLING 配置
- 更新开发环境配置，包含 macOS Docker 环境下的文件监听优化
- 增强 Bun 安装脚本说明，解决容器环境下的依赖安装问题
- 补充文件监控相关环境变量配置说明
...
- **新增**：Docker 环境下文件监控改进，解决 macOS 上的文件监听问题
...
- 构建与开发工具链
  ...
  - Wails：桌面端集成（可选）
- **新增**：文件监控优化
  - VITE_USE_POLLING：前端 Vite 文件轮询监控
  - CHOKIDAR_USEPOLLING：后端 Chokidar 文件轮询监控
  - Bun：增强的包管理器支持
...
D_FE["前端容器 frontend<br/>VITE_USE_POLLING: 'true'"]
D_BE["后端容器 backend<br/>CHOKIDAR_USEPOLLING: 'true'"]
...
- **新增**：Bun 支持
  - 自动检测并安装 Bun，提升包管理性能
  - 在容器环境中通过 dev-bootstrap.sh 脚本处理
...
- 步骤一：安装工作区依赖
  - 在根目录执行：pnpm install
  - pnpm 将根据 pnpm-workspace.yaml 自动链接 apps/_ 与 packages/_
- 步骤二：构建共享库
...
- 全局环境变量
  - NODE*ENV、VITE*\*、WAILS 等在 turbo.json 中声明为全局环境变量
...
- 方式二：Docker 开发模式
  ...
  - 热更新
    - 容器挂载项目目录，修改代码后自动生效
  - **新增**：文件监控优化
    - 前端容器：VITE_USE_POLLING=true，启用文件轮询监控
    - 后端容器：CHOKIDAR_USEPOLLING=true，启用文件轮询监控
...
- 前端代理
  ...
  - **新增**：VITE_USE_POLLING：启用文件轮询监控（默认 false）
- 后端文件监控
  - **新增**：CHOKIDAR_USEPOLLING：启用 Chokidar 文件轮询监控（默认 false）
- Wails 模式
...
**新增**：为了解决 macOS Docker 环境下的文件监听问题，系统引入了文件轮询监控机制：

#### Vite 文件轮询监控

- **配置位置**：docker-compose.dev.yml 中的 frontend 服务
- **环境变量**：VITE_USE_POLLING: 'true'
- **实现原理**：在 Vite 配置中根据环境变量动态启用文件轮询监控
- **适用场景**：macOS 上 Docker Desktop 的文件系统监听不稳定问题

#### Chokidar 文件轮询监控

- **配置位置**：docker-compose.dev.yml 中的 backend 服务
- **环境变量**：CHOKIDAR_USEPOLLING: 'true'
- **实现原理**：通过设置 CHOKIDAR_USEPOLLING 环境变量启用 Chokidar 的轮询模式
- **适用场景**：解决 Docker 环境下文件变更事件丢失的问题

#### 自动化安装增强

- **脚本位置**：scripts/dev-bootstrap.sh
- **功能增强**：自动检测并安装 Bun，提升包管理性能
- **容器支持**：在 Docker 容器环境中正确处理 Bun 安装

```mermaid
graph LR
subgraph "Docker 容器环境"
FE["前端容器<br/>VITE_USE_POLLING: 'true'"]
BE["后端容器<br/>CHOKIDAR_USEPOLLING: 'true'"]
BUN["Bun 包管理器<br/>自动安装"]
end
subgraph "文件监控机制"
POLLING["文件轮询监控"]
FS["文件系统事件"]
...
- **新增**：文件监控依赖
  - 前端 Vite 依赖 VITE_USE_POLLING 环境变量控制文件轮询
  - 后端 NestJS 依赖 CHOKIDAR_USEPOLLING 环境变量控制文件轮询
...
Redis["Redis"] --> Backend
Backend --> Frontend
VitePolling["VITE_USE_POLLING"] --> Frontend
ChokidarPolling["CHOKIDAR_USEPOLLING"] --> Backend
Bun["Bun 包管理器"] --> Frontend
Bun --> Backend
```
...
- **新增**：文件监控性能考虑
  - 文件轮询监控会增加 CPU 使用率，仅在必要时启用
  - 在生产环境中应关闭轮询监控，使用默认的文件系统事件
...
- **新增**：文件监控问题
  - macOS Docker 环境下文件变更不触发：确认 VITE_USE_POLLING 和 CHOKIDAR_USEPOLLING 设置为 'true'
  - 文件轮询导致性能下降：在非 macOS 环境下可关闭轮询监控
  - Bun 安装失败：检查容器网络连接和权限设置
...
**新增**：Docker 环境下的文件监控改进解决了 macOS 用户在 Docker Desktop 上的文件监听问题，通过 VITE_USE_POLLING 和 CHOKIDAR_USEPOLLING 配置确保开发体验的稳定性。
...
- **新增**：文件监控相关
  - VITE_USE_POLLING：前端文件轮询监控开关
  - CHOKIDAR_USEPOLLING：后端文件轮询监控开关
  - Bun：增强的包管理器支持
