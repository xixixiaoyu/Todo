...
- [apps/frontend/src/features/todo/components/TodoBetaBadge.vue](file://apps/frontend/src/features/todo/components/TodoBetaBadge.vue)
...
## 更新摘要
**所做更改**
- 新增 TodoBetaBadge 组件的详细分析和文档说明
- 更新 TodoHeader 组件中 Badge 组件重构变更的说明
- 补充自定义 Badge 组件与通用 Badge 组件的对比分析
- 增强组件复用策略和样式封装的最佳实践指导
...
## 引言
本文件系统性梳理 Lumina Todo 前端组件体系，聚焦 Vue.js 组件架构设计与实践，涵盖功能组件、UI 组件与组合式函数的使用模式；解释命名规范、文件组织与复用策略；阐述组件间通信机制（props、events、provide/inject）；总结组合式函数的设计模式与复用逻辑；给出生命周期管理与性能优化建议；提供组件测试与调试方法；说明样式封装与主题系统集成方式。

**更新** 本次更新重点关注 TodoBetaBadge.vue 新增组件和 TodoHeader.vue 中 Badge 组件重构变更，完善了自定义组件与通用组件的对比分析。
...
BADGE["ui/badge/Badge.vue"]
TODO_BADGE["features/todo/TodoBetaBadge.vue"]
END
subgraph "组合式函数"
...
- [apps/frontend/src/features/todo/components/TodoBetaBadge.vue:1-59](file://apps/frontend/src/features/todo/components/TodoBetaBadge.vue#L1-L59)
...
- UI 组件：通用基础 UI，如 Button、Card、DropdownMenu、Badge 等，强调可复用与一致性。
- 组合式函数：跨组件共享的状态、副作用与逻辑封装，如 useTheme、useRequest、useWindowSize 等。

**更新** 新增 TodoBetaBadge 作为自定义业务组件的代表，展示如何在通用组件基础上进行业务定制。
...
### UI 组件：Badge（徽章）
- 设计要点
  - 通用 Badge 组件提供基础的徽章展示能力，支持变体系统和自定义样式。
  - 通过 badgeVariants 实现统一的视觉规范，支持不同状态下的样式变化。
- 通信与复用
  - 作为通用组件，被多个业务场景复用，保持视觉一致性。
  - 通过 props 接收 variant 和 class，支持灵活的样式定制。

**章节来源**
- [apps/frontend/src/components/ui/badge/Badge.vue:1-18](file://apps/frontend/src/components/ui/badge/Badge.vue#L1-L18)

### 自定义组件：TodoBetaBadge（Todo Beta 徽章）
- 设计要点
  - 基于通用 Badge 组件进行业务定制，专门用于展示 Todo 功能的 Beta 状态。
  - 通过 inheritAttrs: false 禁用属性继承，精确控制传递给底层 Badge 的属性。
  - 内置 outline 变体和特定的样式类名，确保与整体设计语言的一致性。
- 业务特性
  - 包含红色指示点和 "BETA" 文本标签，使用 CSS 变量实现主题适配。
  - 支持明暗主题切换，通过 :global(.dark) 选择器实现深色模式适配。
  - 字体大小、间距、圆角半径等都经过精心调优，符合移动端体验要求。
- 复用策略
  - 作为 Todo 功能域的专用组件，提供明确的业务语义。
  - 通过 Scoped CSS 确保样式隔离，避免与其他组件产生冲突。
  - 支持通过 v-bind="attrs" 透传额外属性，保持与通用 Badge 的兼容性。

```mermaid
classDiagram
class TodoBetaBadgeVue {
+props : "label : string"
+attrs : "inheritAttrs : false"
+template : "Badge(v-bind) + 自定义样式"
+style : "scoped + global(.dark)"
}
class BadgeVue {
+props : "variant?, class?"
+template : "div + badgeVariants"
}
TodoBetaBadgeVue --> BadgeVue : "基于通用 Badge 定制"
```
...
- 组件依赖
  ...
  - 通用 Badge 组件提供基础能力，TodoBetaBadge 基于其进行业务定制。
...
UTILS["cn 工具"] --> BTN
UTILS --> CARD["Card.vue"]
BADGE_CORE["Badge.vue"] --> TODO_BADGE["TodoBetaBadge.vue"]
...
- 组件层面
  ...
  - TodoBetaBadge 通过 inheritAttrs: false 精确控制属性传递，避免不必要的属性透传开销。
- 组合式函数层面
...
**更新** TodoBetaBadge 组件通过 Scoped CSS 和精确的属性控制，在保证功能的同时优化了性能表现。
...
- 组件样式问题
  - 现象：TodoBetaBadge 样式不生效或与其他组件冲突。
  - 处理：检查 Scoped CSS 是否正确应用；确认 CSS 变量是否正确设置；验证 :global(.dark) 选择器的优先级。

**更新** 新增 TodoBetaBadge 组件相关故障排查指导。

**章节来源**
- [apps/frontend/src/main.ts:59-113](file://apps/frontend/src/main.ts#L59-L113)
- [apps/frontend/src/composables/useRequest.ts:28-35](file://apps/frontend/src/composables/useRequest.ts#L28-L35)
- [apps/frontend/src/features/todo/components/TodoBetaBadge.vue:23-58](file://apps/frontend/src/features/todo/components/TodoBetaBadge.vue#L23-L58)
...
**更新** 本次更新完善了自定义组件与通用组件的对比分析，展示了如何在通用组件基础上进行业务定制，为后续类似需求提供了参考模式。

## 附录
- 命名规范与文件组织
  - UI 组件：按功能域分目录，index.ts 聚合导出，组件文件以 PascalCase 命名。
  - 组合式函数：以 use 前缀命名，集中导出至 composables/index.ts。
  - 自定义业务组件：在对应功能域目录下创建专用组件，如 TodoBetaBadge。
  - 根组件与入口：App.vue 与 main.ts 分别承担根组件与应用初始化职责。
- 样式封装与主题系统
  - 通过 CSS 变量与 useTheme 控制主题色与明暗模式；UI 组件通过变体系统与 cn 合并样式，确保一致性与可定制性。
  - 自定义组件采用 Scoped CSS 确保样式隔离，同时支持通过 :global 选择器实现主题适配。
- 组件复用最佳实践
  - 通用组件提供基础能力，自定义组件在通用组件基础上进行业务定制。
  - 通过精确的属性控制和样式封装，确保组件的可复用性和可维护性。
  - 使用 v-bind="attrs" 透传属性时，应明确控制继承范围，避免不必要的属性传递。