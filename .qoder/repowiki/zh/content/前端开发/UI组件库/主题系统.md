...
## 更新摘要
**变更内容**
- 新增 effectiveTheme 和 isDark 计算属性，实现智能主题检测和系统偏好适配
- ThemeToggle 组件现在使用 effectiveTheme 进行图标和标题显示
- useMarkdown 组件监听 effectiveTheme 变化以优化 Mermaid 图表渲染
- 增强了主题检测的用户体验，提供更无缝的深色/浅色模式切换
- 完善了主题系统的响应式设计，支持用户偏好与系统设置的智能协调
...
## 简介
本文件系统性阐述 Lumina Todo 的主题系统，包括颜色体系、主题切换机制、样式变量管理、动态切换与持久化、系统主题适配、CSS 变量使用与覆盖策略、组件级主题定制、深色/浅色差异处理与最佳实践，并提供扩展与品牌定制的实现思路。本次更新重点介绍了新增的智能主题检测功能：useTheme 组合式函数现在包含 effectiveTheme 和 isDark 计算属性，能够尊重用户偏好同时自动检测系统深色模式设置，提供更无缝的用户体验。

**更新** 本版本反映了最新的智能主题检测增强功能，包括 effectiveTheme 和 isDark 计算属性的实现，ThemeToggle 组件的相应更新，以及 useMarkdown 组件对主题变化的响应优化。
...
- [apps/frontend/src/features/todo/components/ThemeColorPicker.vue:16](file://apps/frontend/src/features/todo/components/ThemeColorPicker.vue#L16)
...
## 结论
Lumina Todo 的主题系统以 CSS 变量为核心，结合运行时组合式函数实现"可定制、可持久化、可扩展"的主题体验。通过语义化变量与深/浅两套覆盖，兼顾可访问性与视觉舒适度；通过随机主题色与定时切换，平衡个性化与专注力。新增的智能主题检测功能进一步提升了系统的用户体验，通过 effectiveTheme 和 isDark 计算属性实现用户偏好与系统设置的智能协调，确保主题切换的无缝体验。无障碍功能包括自动对比度计算、智能前景色选择和设备能力感知的智能悬停管理，整体架构清晰、耦合度低，便于进一步扩展品牌色与组件级主题定制。

**更新** 最新的智能主题检测功能显著增强了系统的可访问性和用户体验，effectiveTheme 和 isDark 计算属性确保用户偏好始终被优先考虑，同时智能检测系统深色模式设置。ThemeToggle 组件的相应更新提供了更直观的主题切换体验，useMarkdown 组件对主题变化的监听优化了渲染性能。Lilac Gray 颜色值的更正和推荐指示器的文本化改进进一步提升了视觉一致性和用户体验。
...
- [apps/frontend/tests/features/todo/components/ThemeColorPicker.spec.ts:1-155](file://apps/frontend/tests/features/todo/components/ThemeColorPicker.spec.ts#L1-L155)
- [apps/frontend/tests/features/todo/components/ThemeToggle.spec.ts:1-76](file://apps/frontend/tests/features/todo/components/ThemeToggle.spec.ts#L1-L76)
...
这些翻译确保了推荐主题指示器和主题切换按钮在不同语言环境下的本地化体验。
