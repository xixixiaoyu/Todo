...
## 更新摘要
**变更内容**
- 更新 Lilac Gray 颜色值从 `#918aa1` 到 `#8e81c0` 的记录
- 更新主题预设颜色列表中的 Lilac Gray 颜色值
- 更新测试用例中对 Lilac Gray 颜色值的断言
- 更新国际化文件中对 Lilac Gray 的翻译
...
**更新** 本版本反映了 Lilac Gray 颜色值从 `#918aa1` 更新为 `#8e81c0` 的最新变更。
...
- **Celadon（青瓷）**：`#78958e` - 清新绿松石色调，适合专注工作场景
- **Twilight Amber（暮色）**：`#9b8574` - 温暖琥珀色调，营造舒适氛围
- **Mist Blue（薄雾）**：`#728ba1` - 柔和蓝灰色，平衡冷静与温暖
- **Moss Green（苔青）**：`#7c9585` - 深邃绿色，提升自然亲和感
- **Lilac Gray（丁香）**：`#8e81c0` - 更新后的紫色灰调，从 `#918aa1` 调整为 `#8e81c0`
- **Sunset Rose（晚霞）**：`#ae8792` - 柔和玫瑰色调，温暖而优雅
- **Graphite（石墨）**：`#647587` - 经典深灰色，专业且稳重
- **Indigo（靛青）**：`#4f6284` - 深邃蓝色，提升专业感

**更新** Lilac Gray 颜色值已从 `#918aa1` 更新为 `#8e81c0`，这一调整提供了更柔和的紫色灰调，改善了视觉舒适度和可访问性。

**章节来源**
- [apps/frontend/src/composables/useTheme.ts:53-63](file://apps/frontend/src/composables/useTheme.ts#L53-L63)
- [apps/frontend/tests/features/todo/components/ThemeColorPicker.spec.ts:42-72](file://apps/frontend/tests/features/todo/components/ThemeColorPicker.spec.ts#L42-L72)
...
**更新** 最新的 Lilac Gray 颜色调整体现了我们对用户体验细节的关注，新的 `#8e81c0` 颜色值提供了更好的视觉平衡和可访问性。
...
**更新** 测试用例已更新以反映 Lilac Gray 颜色值从 `#918aa1` 到 `#8e81c0` 的变更。
