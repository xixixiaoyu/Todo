...
## 更新摘要
**变更内容**
- 新增专用的WebSocket错误处理工具函数模块
- 实现错误分类机制：认证相关错误 vs 临时网络问题
- 添加错误日志冷却机制，避免重复日志
- 优化断线重连策略，自动处理认证错误
...
ERRORS["错误处理工具<br/>useSocket.errors.ts"]
...
- **新增** 专用错误处理工具：提供认证错误与临时网络错误的智能分类，支持错误日志冷却机制。
...
participant ERR as "错误处理工具<br/>useSocket.errors.ts"
...
- **更新** 错误处理与重连策略
  - 专用错误分类：通过 isSocketAuthError 和 isTransientSocketError 函数区分错误类型
  - 认证错误自动处理：检测到认证相关错误时，自动刷新访问令牌并重新连接
  - 临时错误过滤：网络临时性错误被过滤，避免产生噪音日志
  - 错误日志冷却：重复的意外错误在10秒内被抑制，防止日志风暴
- 连接等待
...
participant ERR as "useSocket.errors.ts"
...
GW-->>US : "加入成功"
SIO-->>US : "disconnect 或 connect_error"
US->>ERR : "分类错误类型"
ERR-->>US : "认证错误/临时错误"
alt 认证错误
US->>US : "刷新访问令牌"
...
### 专用错误处理工具
**新增功能** 专门的错误处理模块提供了以下能力：
- 错误类型分类
  - 认证相关错误：包含 "unauthorized"、"token"、"jwt"、"authentication" 关键词
  - 临时网络错误：包含 "timeout"、"transport close"、"transport error"、"websocket error"、"xhr poll error"、"xhr post error" 关键词
- 智能检测机制
  - 不区分大小写的关键字匹配
  - 支持检查错误消息和错误类型字段
- 错误日志优化
  - 10秒冷却时间，避免重复错误日志
  - 错误指纹识别，相同类型的错误在冷却期内被抑制
...
- **新增** 错误处理工具为 Socket 组合式函数提供专用的错误分类能力
...
USOCK --> ERRORS["useSocket.errors.ts"]
ERRORS --> AUTH["auth store"]
...
- **新增** 错误处理问题
  - 认证错误未自动处理：检查 isSocketAuthError 分类逻辑
  - 临时错误仍被记录：检查 isTransientSocketError 过滤逻辑
  - 错误日志被过度抑制：检查冷却时间配置
...
## 结论
本架构以 Socket.IO 为核心，结合严格的认证与房间权限控制、事务化的增量同步与冲突检测、以及广播防抖与指数退避重连策略，实现了高可用、低耦合、可扩展的实时通信体系。**最新的改进**通过专用的错误处理工具实现了智能错误分类与自动认证错误恢复，显著提升了系统的鲁棒性和用户体验。通过共享 Schema 统一契约，前后端协作清晰，具备良好的可维护性与扩展性。
