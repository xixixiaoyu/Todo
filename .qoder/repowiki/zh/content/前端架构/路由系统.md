...
- 增强认证保护机制，为 MCP 设置、小说创作、教学管理等路由添加认证要求
...
- [apps/frontend/src/router/index.ts:1-116](file://apps/frontend/src/router/index.ts#L1-L116)
...
- 路由表
  ...
  - 小说创作相关路径 '/novel' 和 '/novel/:id' 设置 `requiresAuth: true`
  - 教学管理路径 '/teaching' 设置 `requiresAuth: true`
  - 通配符路径 '/:pathMatch(.*)*' 匹配所有未识别路径，渲染 404 页面
...
**图表来源**
- [apps/frontend/src/main.ts:10-138](file://apps/frontend/src/main.ts#L10-L138)
- [apps/frontend/src/router/index.ts:77-113](file://apps/frontend/src/router/index.ts#L77-L113)
...
**章节来源**
- [apps/frontend/src/router/index.ts:7-10](file://apps/frontend/src/router/index.ts#L7-L10)
...
- 路由表结构
  ...
  - 小说创作路由：'/novel' 和 '/novel/:id' 懒加载，设置 `requiresAuth: true`
  - 教学管理路由 '/teaching' 懒加载，设置 `requiresAuth: true`
  - 通配符路由 '/:pathMatch(.*)*' 懒加载 404 页面
- 懒加载与代码分割
  - 所有视图组件均通过动态 import 实现懒加载
  - Vite 的 manualChunks 将常用第三方库归类到固定 chunk 名称，减少碎片化并提升缓存效率

**章节来源**
- [apps/frontend/src/router/index.ts:11-74](file://apps/frontend/src/router/index.ts#L11-L74)
- [apps/frontend/vite.config.mts:23-78](file://apps/frontend/vite.config.mts#L23-L78)
...
**章节来源**
- [apps/frontend/src/router/index.ts:77-91](file://apps/frontend/src/router/index.ts#L77-L91)
- [apps/frontend/src/i18n/index.ts:1-37](file://apps/frontend/src/i18n/index.ts#L1-L37)
- [apps/frontend/tests/router/index.spec.ts:39-68](file://apps/frontend/tests/router/index.spec.ts#L39-L68)
...
**章节来源**
- [apps/frontend/src/router/index.ts:93-113](file://apps/frontend/src/router/index.ts#L93-L113)
- [apps/frontend/src/features/auth/stores/auth.ts:35](file://apps/frontend/src/features/auth/stores/auth.ts#L35)
- [apps/frontend/src/composables/useToast.ts:77-85](file://apps/frontend/src/composables/useToast.ts#L77-L85)
...
### 动态路由与通配符路由
- 动态路由
  - 当前路由表定义了具名参数的动态段：'/novel/:id'
  - 动态参数通过 `$route.params.id` 获取
- 通配符路由
  - 使用 '/:pathMatch(.*)*' 捕获所有未匹配路径，统一跳转至 404 页面
  - 404 页面通过 RouterLink 返回首页，提供友好的用户体验

**章节来源**
- [apps/frontend/src/router/index.ts:57-61](file://apps/frontend/src/router/index.ts#L57-L61)
...
**章节来源**
- [apps/frontend/src/router/index.ts:48-66](file://apps/frontend/src/router/index.ts#L48-L66)
...
**章节来源**
- [apps/frontend/src/router/index.ts:21-27](file://apps/frontend/src/router/index.ts#L21-L27)
- [apps/frontend/src/i18n/index.ts:1-37](file://apps/frontend/src/i18n/index.ts#L1-L37)
...
## 结论
Lumina Todo 的前端路由系统以 Vue Router 为核心，结合 Vite 的代码分割与 PWA 能力，实现了简洁高效的页面导航与性能优化。**最新更新**增强了路由守卫系统，新增全局导航守卫检查 `requiresAuth` 元信息，为 MCP 设置、小说创作、教学管理等路由提供了更好的认证保护和用户体验。当前路由表采用扁平结构与懒加载策略，导航守卫负责页面标题的国际化更新和认证拦截。未来可在路由层引入更细粒度的权限控制与嵌套路由，进一步完善权限控制与布局复用；同时建议在需要场景中使用查询参数与书签化路由，以提升 SEO 与可访问性。
