...
F --> H["Todo 视图<br/>apps/frontend/src/features/todo/TodoView.vue"]
F --> I["登录视图<br/>apps/frontend/src/features/auth/views/LoginView.vue"]
F --> J["MCP 设置视图<br/>apps/frontend/src/features/mcp/views/McpSettingsView.vue"]
F --> K["404 页面<br/>apps/frontend/src/views/error/NotFoundView.vue"]
```
...
章节来源
- [apps/frontend/src/router/index.ts:11-53](file://apps/frontend/src/router/index.ts#L11-L53)
- [apps/frontend/vite.config.mts:23-78](file://apps/frontend/vite.config.mts#L23-L78)
...
- [apps/frontend/src/router/index.ts:56-70](file://apps/frontend/src/router/index.ts#L56-L70)
- [apps/frontend/src/i18n/index.ts:1-37](file://apps/frontend/src/i18n/index.ts#L1-L37)
- [apps/frontend/tests/router/index.spec.ts:25-54](file://apps/frontend/tests/router/index.spec.ts#L25-L54)
...
### 动态路由与通配符路由
- 动态路由
  - 当前路由表未定义具名参数的动态段（如 '/user/:id'）
  - 如需动态参数，可在路由定义中添加相应路径段，并在组件中通过 $route.params 获取
- 通配符路由
  - 使用 '/:pathMatch(.*)*' 捕获所有未匹配路径，统一跳转至 404 页面
  - 404 页面通过 RouterLink 返回首页，提供友好的用户体验

章节来源
- [apps/frontend/src/router/index.ts:48-52](file://apps/frontend/src/router/index.ts#L48-L52)
...
- 查询字符串处理
  ...
  - 建议在需要分享或书签化的场景中，将关键参数映射到路由查询，便于 SEO 与可访问性
- 路由历史
...
- 当前实现
  - 路由层未实现基于角色或令牌的守卫逻辑
  - 登录成功后的跳转在视图组件中完成（例如登录页提交成功后 router.push('/')）
- 建议方案
  - 在 beforeEach 中加入鉴权逻辑：校验登录状态与所需权限，必要时重定向至登录页
  - 结合路由元信息（如 roles、requiresAuth）细化权限控制粒度
  - 对需要登录才能访问的页面，在 beforeEach 中统一拦截
...
## 结论
Lumina Todo 的前端路由系统以 Vue Router 为核心，结合 Vite 的代码分割与 PWA 能力，实现了简洁高效的页面导航与性能优化。当前路由表采用扁平结构与懒加载策略，导航守卫负责页面标题的国际化更新。未来可在路由层引入权限守卫与嵌套路由，进一步完善权限控制与布局复用；同时建议在需要场景中使用查询参数与书签化路由，以提升 SEO 与可访问性。
