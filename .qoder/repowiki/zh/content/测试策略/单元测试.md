...
- [apps/frontend/tests/composables/markdown/preprocessor.spec.ts](file://apps/frontend/tests/composables/markdown/preprocessor.spec.ts)
- [apps/frontend/tests/styles/markdown-theme.spec.ts](file://apps/frontend/tests/styles/markdown-theme.spec.ts)
- [apps/frontend/tests/composables/useMarkdown.spec.ts](file://apps/frontend/tests/composables/useMarkdown.spec.ts)
- [apps/frontend/tests/composables/markdown/mermaid.spec.ts](file://apps/frontend/tests/composables/markdown/mermaid.spec.ts)
- [apps/frontend/tests/features/ai/composables/useChatMessageMarkdownRender.spec.ts](file://apps/frontend/tests/features/ai/composables/useChatMessageMarkdownRender.spec.ts)
- [apps/frontend/src/composables/markdown/preprocessor.ts](file://apps/frontend/src/composables/markdown/preprocessor.ts)
- [apps/frontend/src/composables/markdown/plugins.ts](file://apps/frontend/src/composables/markdown/plugins.ts)
</cite>

## 更新摘要
**所做更改**
- 新增了LaTeX预处理器测试章节，涵盖数学公式标准化、任务列表渲染和Markdown主题样式测试
- 扩展了Markdown渲染测试覆盖范围，包括数学公式渲染、任务列表处理和Mermaid图表支持
- 增强了前端Markdown组件测试，涵盖预处理器、渲染器和主题样式的全面测试
- 新增了AI聊天消息Markdown渲染测试，支持流式渲染和交互功能
...
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [LaTeX预处理器测试](#latex预处理器测试)
7. [Markdown渲染测试](#markdown渲染测试)
8. [依赖分析](#依赖分析)
9. [性能考虑](#性能考虑)
10. [故障排查指南](#故障排查指南)
11. [结论](#结论)
...
**更新** 本次更新显著增强了测试覆盖范围，新增了29+个LaTeX预处理器测试用例和49+个markdown渲染测试用例，重点加强了数学公式渲染、任务列表处理和Markdown主题样式的测试基础设施。
...
### 数学公式标准化测试
新增的LaTeX预处理器测试涵盖了数学公式的多种标准化场景，确保AI生成的数学内容能够正确渲染。

**章节来源**
- [apps/frontend/tests/composables/markdown/preprocessor.spec.ts:1-121](file://apps/frontend/tests/composables/markdown/preprocessor.spec.ts#L1-L121)

### LaTeX环境处理测试
测试覆盖了裸露LaTeX环境块的处理，包括align、equation等环境的正确识别和转换。

**章节来源**
- [apps/frontend/tests/composables/markdown/preprocessor.spec.ts:75-89](file://apps/frontend/tests/composables/markdown/preprocessor.spec.ts#L75-L89)

### 任务列表渲染测试
Markdown主题测试专门验证任务列表的样式渲染，确保复选框标记正确显示为清单项。

**章节来源**
- [apps/frontend/tests/styles/markdown-theme.spec.ts:50-61](file://apps/frontend/tests/styles/markdown-theme.spec.ts#L50-L61)

## Markdown渲染测试

### 数学公式渲染测试
useMarkdown组合式函数的测试涵盖了从标准LaTeX定界符到裸露环境块的完整数学公式渲染流程。

**章节来源**
- [apps/frontend/tests/composables/useMarkdown.spec.ts:41-76](file://apps/frontend/tests/composables/useMarkdown.spec.ts#L41-L76)

### 任务列表处理测试
测试验证了Markdown任务列表的正确解析和渲染，包括复选框状态和样式应用。

**章节来源**
- [apps/frontend/tests/composables/useMarkdown.spec.ts:109-118](file://apps/frontend/tests/composables/useMarkdown.spec.ts#L109-L118)

### Mermaid图表渲染测试
Mermaid图表的测试包括安全初始化、SVG净化和主题CSS注入等功能。

**章节来源**
- [apps/frontend/tests/composables/markdown/mermaid.spec.ts:8-34](file://apps/frontend/tests/composables/markdown/mermaid.spec.ts#L8-L34)

### AI聊天消息渲染测试
AI聊天消息的Markdown渲染测试支持流式渲染、选择检测和Mermaid图表交互功能。

**章节来源**
- [apps/frontend/tests/features/ai/composables/useChatMessageMarkdownRender.spec.ts:87-130](file://apps/frontend/tests/features/ai/composables/useChatMessageMarkdownRender.spec.ts#L87-L130)
...
- 外部依赖
  ...
  - 数学公式渲染依赖@iktakahiro/markdown-it-katex和katex库。
...
- [apps/backend/tests/auth/auth.service.spec.ts:10-32](file://apps/backend/tests/auth/auth.service.spec.ts#L10-L32)
- [apps/backend/tests/todos.service.spec.ts:32-47](file://apps/backend/tests/todos.service.spec.ts#L32-L47)
- [apps/frontend/tests/composables/markdown/preprocessor.spec.ts:40-89](file://apps/frontend/tests/composables/markdown/preprocessor.spec.ts#L40-L89)
...
- 常见问题
  ...
  - 数学公式渲染错误：检查katex版本兼容性和LaTeX语法正确性。
  - 任务列表样式问题：验证CSS变量和主题配置是否正确应用。
- 排查步骤
  - 在 beforeEach 中重置所有 mocks 与全局桩；
  - 使用 spy 观察方法调用次数与参数；
  - 对异步流程使用超时与断言顺序验证；
  - 对 E2E 场景，先断言状态码再断言响应体字段；
  - 检查LaTeX预处理器的正则表达式匹配和代码块保护机制。
...
## 结论
本仓库的测试体系以 Vitest 为核心，结合后端 Nest 与前端 Vue 生态，形成了从单元到端到端的完整测试金字塔。通过合理的 Mock/Stub、依赖注入与全局 setup，实现了高隔离、高可读与高可维护的测试实践。

**更新** 本次更新显著增强了测试覆盖范围，新增的LaTeX预处理器和Markdown渲染测试确保了数学公式、任务列表和主题样式的正确性。建议在新增功能时遵循现有模式：优先单元测试，其次集成测试，最后补充必要的端到端场景，并持续关注覆盖率与性能指标。
...
- 注入
  ...
  - Mermaid图表测试中使用hoisted mocks确保模块隔离。
...
- 数学公式和任务列表测试应覆盖边界情况和错误输入；
- 前端Markdown测试应包含主题切换和响应式布局验证。