...
## 更新摘要
**变更内容**
- 更新了测试套件部分，反映认证系统测试的重大改进
- 新增了复杂的刷新令牌测试逻辑和错误场景覆盖
- 增强了前端状态管理的重试机制测试
- 完善了单次使用mock行为的测试实现
...
9. [故障排查指南](#故障排查指南)
10. [结论](#结论)
11. [附录：API 使用示例与最佳实践](#附录api-使用示例与最佳实践)
...
subgraph "测试套件"
BE_Tests["后端测试套件<br/>覆盖所有认证场景"] --> FE_Tests["前端测试套件<br/>包含重试机制验证"]
end
...
- 当前用户装饰器(CurrentUser)
  - 从请求对象提取当前用户，支持按字段取值。
- 前端状态管理(useAuthStore)
  - 管理 token/refreshToken/user/错误与加载态；封装登录/注册/刷新/登出/密码重置；持久化与跨页面恢复；实现复杂的重试机制。
...
## 结论
Lumina Todo 的认证系统通过"短期访问令牌 + 长期刷新令牌"的双令牌模型，配合会话失效与黑名单机制，在保证安全性的同时兼顾了用户体验。后端以 NestJS 的 Passport/JWT 为核心，前端以 Pinia 管理状态并实现稳健的令牌轮转与错误处理。**最新的测试套件改进进一步增强了系统的可靠性，包含了复杂的重试机制、精确的mock行为和全面的错误场景覆盖**。整体架构清晰、职责分离明确，适合在生产环境中部署与扩展。
