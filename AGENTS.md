...
## 项目上下文（中文优先）
- 在本仓库执行任何任务前，必须先阅读本文件（`AGENTS.md`）。
- `AGENTS.md` 是本仓库工程规则的单一规范源（SSOT）。
- 本文件包含 AI 入口导航、执行风格提示以及详细的工程开发规范。

## 优先级说明
- 系统级或平台级指令优先于本仓库文档。
- 仓库文档内若出现工程规则冲突，以本文件（`AGENTS.md`）为准。
...
- 指令优先级：系统/平台级指令 > 本文件（`AGENTS.md`） > 其他普通说明
- 关系补充：本文件为工程规则 SSOT
...
- 行为变更必须有可验证的回归防护，按影响面分级：
  - 共享契约（Schema/DTO/类型）：必须补齐测试，无测试不交付
  - 后端业务逻辑（Service 层、数据持久化、鉴权）：必须补齐测试，无测试不交付
  - 前端关键路径（状态管理、数据流、API 编排）：必须补齐测试
  - 前端 UI 交互（纯样式/布局/动效调整、DOM 操作细节）：评估风险后酌情覆盖，低风险场景允许免测提交
...
- 行为基调：博学、严谨、务实；先给结论再展开原因，先指架构问题再说语法细节
- 工程标准：坚持最佳实践、拒绝 Hack、默认防御式编程、务实抽象（Rule of Three）、保持逻辑扁平
- 代码美学：强类型优先（禁止 `any`、优先 `interface`）、遵循 2 空格/单引号/无分号
- 视觉要求：UI 遵循项目设计系统规范，兼顾性能（避免大面积 layout thrashing）与 A11y
...
- 是否严格遵守依赖边界，未引入跨层污染
- 是否按分级规则补齐了受影响行为的测试（共享层/后端业务逻辑必须，前端 UI 酌情）
- 是否同步更新了共享契约、类型与调用方
- 是否验证了 API 响应结构与错误语义
